...
try:
    from pykakasi import kakasi,wakati # 'wakati' nếu cần tách từ (có thể không cần ngay)

    kks = kakasi(
    )

    kakasi_converter = kks # Gán instance kakasi
    # Kiểm tra nhanh xem convert có hoạt động không
    try:
...
except Exception as e: # Bắt các lỗi khác có thể xảy ra khi khởi tạo Kakasi
    kakasi_converter = None
...
bot.wiktionary_cache_vn = {} # Cache Wiktionary VN
bot.wiktionary_cache_jp = {} # Cache Wiktionary JP
bot.local_dictionary_vn = set() # Từ điển local VN
bot.local_dictionary_jp = [] # Từ điển local JP, list of dicts: [{'kanji': '愛育', 'hira': 'あいいく', 'roma': 'aiiku'}, ...]
bot.kakasi = kakasi_converter # Gán instance kakasi vào bot


# --- TẢI TỪ ĐIỂN LOCAL ---
async def load_vietnamese_dictionary(bot_instance: commands.Bot, file_path="exp-tudien-vn.txt"): # Đổi tên file
...
async def load_japanese_dictionary(bot_instance: commands.Bot, file_path="exp-wordlist_jp.txt"):
    script_dir = os.path.dirname(__file__)
    absolute_file_path = os.path.join(script_dir, file_path)
    loaded_count = 0
    try:
        with open(absolute_file_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                if len(row) >= 3:
                    kanji, hira, roma = row[0].strip(), row[1].strip(), row[2].strip()
                    # Chỉ thêm nếu có Hiragana (là dạng chuẩn để so sánh)
                    if hira:
                        entry = {'kanji': kanji if kanji else hira, 'hira': hira, 'roma': roma}
...
async def on_ready():
    ...
    if not bot.db_pool:
        bot.db_pool = await database.init_db(
            ...
            bot_cfg.DEFAULT_GAME_LANGUAGE # Truyền default language
        )
    ...
    # Lấy application_id từ application_info() nếu chưa có
    if not bot.application_id and not bot_cfg.APPLICATION_ID:
        try:
            app_info = await bot.application_info()
            bot_cfg.APPLICATION_ID = app_info.id # Lưu vào config để các nơi khác dùng nếu cần
            bot.application_id = app_info.id # Lưu trực tiếp vào bot instance
        except Exception as e:
    ...
    if bot.db_pool:
        ...
        try:
            # Kiểm tra xem bot có application_id không trước khi sync
            if bot.application_id:
    ...
    print(f"Ngôn ngữ game mặc định: {bot_cfg.DEFAULT_GAME_LANGUAGE}.")
...
async def main():
    ...
    if not bot.kakasi: # Cảnh báo nếu Kakasi không chạy được
