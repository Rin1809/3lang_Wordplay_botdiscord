...
from .. import wiktionary_api # Đã sửa để nhận local_dictionary
from .. import utils
    ...
    if not start_phrase_input: # Bot chọn từ
        ...
        for phrase_attempt in possible_starts: # Tìm từ hợp lệ
            if await wiktionary_api.is_vietnamese_phrase_or_word_valid_api(
                phrase_attempt, bot.http_session, bot.wiktionary_cache, bot.local_dictionary # Truyền dict local
            ):
        ...
        if not await wiktionary_api.is_vietnamese_phrase_or_word_valid_api(
            current_phrase_normalized, bot.http_session, bot.wiktionary_cache, bot.local_dictionary # Truyền dict local
        ):
            msg = f"⚠️ Cụm từ \"**{original_input_display.title()}**\" không hợp lệ theo từ điển." # Sửa msg
            await send_response(msg, ephemeral_flag=True)
            return
...
async def process_game_message(bot: commands.Bot, message: discord.Message):
    ...
    # Kiểm tra từ điển nếu chưa có lỗi
    if not error_occurred and not await wiktionary_api.is_vietnamese_phrase_or_word_valid_api(
        user_phrase_normalized, bot.http_session, bot.wiktionary_cache, bot.local_dictionary # Truyền dict local
    ):
        error_occurred = True; error_type_for_stat = "invalid_wiktionary" # Vẫn dùng key này cho stat
